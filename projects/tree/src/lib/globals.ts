...
export class Globals {
    ...
    filterTree(tree: Node[], keyword: string): Node[] {
        ...
        return tree.filter(function filterCallback(node) {
            ...
            if (node.Name.toLocaleLowerCase().indexOf(keyword) > -1) {
                ...
                if (node.Children) {
                    // //  Recursive calls to the filter function to dig down in children, grand children ..etc
    ...
    //  ShowMatchesAndOpenNoFilterCallBack(node: Node, keyword: string): boolean {
    //                 if (node.Children) {
    //                     for (let child of node.Children) {
    //                         if (child.Name.toLocaleLowerCase().indexOf(keyword) > -1) {
    //                             child.KeywordMatch = true;
    //                             node.IsOpen = true;
    //                         }
    //                         this.ShowMatchesAndOpenNoFilterCallBack(child);
    //                     }
    //                 }
    // }
}
